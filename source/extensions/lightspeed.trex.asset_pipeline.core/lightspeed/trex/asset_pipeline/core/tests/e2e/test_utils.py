...
class TestAssetPublicationE2E(omni.kit.test.AsyncTestCase):
    ...
    async def test_remote_batch_copy_failure_preserves_previous_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(RuntimeError) as error:
                await utils_module._publish_remote_batch([first_source, missing_source], local_dir, remote_url)
    ...
    async def test_remote_commit_and_staging_cleanup_failure_restores_previous_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            remote_url = omni.client.make_file_url(str(remote_dir))
            real_move_async = utils_module.move_async
            real_delete_remote_path = utils_module._delete_remote_path
            ...
            with (
                mock.patch.object(utils_module, "move_async", side_effect=fail_commit),
                mock.patch.object(utils_module, "_delete_remote_path", side_effect=fail_staging_cleanup),
                ...
                await utils_module._publish_remote_batch([new_source], local_dir, remote_url)
    ...
    async def test_cancellation_after_backup_move_restores_previous_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(utils_module, "move_async", side_effect=pause_after_backup_move):
                # Pause immediately after backup creation, then cancel the in-flight publication.
                publication_task = asyncio.create_task(
                    utils_module._publish_remote_batch([new_source], local_dir, remote_url)
                )
    ...
    async def test_cancellation_after_first_commit_removes_new_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(utils_module, "move_async", side_effect=pause_after_final_move):
                # Commit the first real batch remotely, then cancel before the settled result reaches the caller.
                publication_task = asyncio.create_task(
                    utils_module._publish_remote_batch([source], local_dir, remote_url)
                )
    ...
    async def test_cancellation_during_competing_first_commit_preserves_other_destination(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(utils_module, "move_async", side_effect=create_competing_destination):
    ...
    async def test_competing_replacement_preserves_other_destination(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                mock.patch.object(utils_module, "move_async", side_effect=create_competing_destination),
                self.assertRaises(RuntimeError),
            ):
                # Let another publisher replace the destination after this transaction retains its previous version.
                await utils_module._publish_remote_batch([source], local_dir, remote_url)
    ...
    async def test_mesh_publication_retains_prior_internal_texture_batch(self):
        """The terminal mesh batch creates missing parents and keeps prerequisite internal textures."""
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create separate texture and mesh batches for a remote directory that does not exist.
            temp_path = pathlib.Path(temp_dir)
            texture_batch = temp_path / "texture_batch"
            internal_texture = texture_batch / "textures" / "internal.rtex"
            internal_texture.parent.mkdir(parents=True)
            internal_texture.write_bytes(b"internal texture")
            mesh_batch = temp_path / "mesh_batch"
            model = mesh_batch / "model.usd"
            mesh_batch.mkdir()
            model.write_bytes(b"optimized model")
            remote_dir = temp_path / "missing" / "parents" / "remote"
            remote_url = omni.client.make_file_url(str(remote_dir))

            # Publish the internal texture batch before the terminal mesh batch.
            await utils_module._publish_remote_batch([internal_texture], texture_batch, remote_url)
            await utils_module._publish_remote_batch([model], mesh_batch, remote_url)

            # Both assets remain in the remote directory, with no transaction paths.
            self.assertEqual((remote_dir / "textures" / "internal.rtex").read_bytes(), b"internal texture")
            self.assertEqual((remote_dir / "model.usd").read_bytes(), b"optimized model")
            self.assertEqual(list(remote_dir.parent.glob("remote.asset_pipeline_*")), [])
