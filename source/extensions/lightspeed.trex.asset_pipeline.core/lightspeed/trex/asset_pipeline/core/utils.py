...
def resolve_local_output_dir(job_directory: pathlib.Path, output_url: str | None) -> tuple[pathlib.Path, bool]:
    """Return the local directory the pipeline writes to and whether the outputs need remote publication.

    A local ``output_url`` is used directly. A missing or remote ``output_url`` routes the outputs to the
    job's ``processed`` directory; a remote one is then published from there.
    """
    if output_url is not None:
        local_output_dir = get_local_path(output_url)
...
def get_authoring_spec(attr: Usd.Attribute, model_parent: pathlib.Path) -> Sdf.AttributeSpec:
    """Return the strongest authored spec for an attribute that this model owns.

    Args:
        attr: Attribute whose authored value will be changed.
        model_parent: Directory that contains every layer this operation may change.

    Returns:
        The strongest authored attribute spec.

    Raises:
        RuntimeError: If the attribute has no real authoring layer inside the model directory.
    """
    property_stack = attr.GetPropertyStack(Usd.TimeCode.Default())
    authoring_spec = property_stack[0] if property_stack else None
    authoring_layer = authoring_spec.layer if authoring_spec is not None else None
    if authoring_spec is None or authoring_layer is None or not authoring_layer.realPath:
        raise RuntimeError(f"Cannot resolve an authoring layer for {attr.GetPath()}")

    authoring_layer_path = pathlib.Path(authoring_layer.realPath).resolve()
    try:
        authoring_layer_path.relative_to(model_parent)
    except ValueError as error:
        raise RuntimeError(
            f"Refusing to rewrite {attr.GetPath()} authored outside the model directory: {authoring_layer_path}"
        ) from error
...
async def _ensure_remote_parent_directories(url: str) -> None:
    """Create each missing parent directory from the nearest existing ancestor."""
    parts = break_url(url)
    parent_path = pathlib.PurePosixPath(parts.path).parent
    missing_urls = []
    while parent_path != parent_path.parent:
        parent_url = make_url(scheme=parts.scheme, host=parts.host, path=parent_path.as_posix())
        stat_result, _entry = await await_settled(stat_async(parent_url))
        if stat_result == ClientResult.OK:
            break
        if stat_result != ClientResult.ERROR_NOT_FOUND:
            raise RuntimeError(f"Could not inspect asset publication parent {parent_url} ({stat_result})")
        missing_urls.append(parent_url)
        parent_path = parent_path.parent

    for parent_url in reversed(missing_urls):
        create_result = await await_settled(create_folder_async(parent_url))
        if create_result == ClientResult.OK:
            continue
        stat_result, _entry = await await_settled(stat_async(parent_url))
        if stat_result != ClientResult.OK:
            _require_remote_success(create_result, f"create asset publication parent {parent_url}")
...
async def _publish_remote_batch(
    ...
    try:
        await _ensure_remote_parent_directories(final_url)
