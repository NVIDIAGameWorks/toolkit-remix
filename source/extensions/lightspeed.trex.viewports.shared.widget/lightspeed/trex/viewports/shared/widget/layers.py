...
class ViewportLayers:
    """
    A widget that holds a single viewport and manages the order of layers within a ui.ZStack
    """
    ...
    def __init__(
        ...
        **ui_kwargs,
        ...
        # Expose the viewport itself into the layer system (factory is the key, so use the constructor)
    ...
    def find_viewport_layer(self, layer_id: str, category: str = None, layers=None) -> _LayerItem | None:
        def recurse_layers(layer):
            if layer_id == getattr(layer, "name", None) and (
                category is None or category in getattr(layer, "categories", ())
            ):
                return layer
