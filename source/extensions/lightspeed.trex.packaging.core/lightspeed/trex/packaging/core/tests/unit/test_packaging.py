...
from lightspeed.layer_manager.constants import (
...
class TestPackagingCoreUnit(omni.kit.test.AsyncTestCase):
    ...
    async def test_package_no_root_layer_should_warn_and_quick_return(self):
        ...
        with patch("lightspeed.trex.packaging.core.packaging._ModPackagingSchema") as model_mock, patch.object(
            PackagingCore, "_initialize_usd_stage"
        ) as init_usd_mock, patch.object(carb, "log_warn") as warn_mock:
    ...
    async def test_package_no_exported_mod_layer_should_add_error(self):
        ...
        with patch("lightspeed.trex.packaging.core.packaging._ModPackagingSchema") as model_mock, patch.object(
            PackagingCore, "_initialize_usd_stage"
        ) as init_usd_mock, patch.object(PackagingCore, "_filter_sublayers") as filter_mock, patch.object(
            PackagingCore, "_get_redirected_dependencies"
        ) as redirect_mock, patch.object(
            PackagingCore, "_make_temp_layer"
        ) as make_temp_mock, patch.object(
            PackagingCore, "_collect"
        ) as collect_mock, patch.object(
            PackagingCore, "_update_layer_metadata"
        ) as update_metadata_mock, patch.object(
            PackagingCore, "_packaging_completed"
    ...
    async def test_package_should_filter_redirect_and_collect_then_trigger_packaging_complete_event(self):
        ...
        with patch("lightspeed.trex.packaging.core.packaging._ModPackagingSchema") as model_mock, patch.object(
            ...
            Sdf.Layer, "FindOrOpen"
        ) as find_open_mock, patch.object(
    ...
    async def test_filter_sublayers_should_recursively_remove_non_selected_sublayers_and_return_remaining_layers(self):
        ...
        ) as get_original_mock, patch.object(
            LayerUtils, "get_sublayer_position_in_parent"
        ) as get_position_mock, patch.object(
    ...
    async def __run_collect(self, should_cancel: bool, has_unresolved_assets: bool):
        ...
        ) as compute_dependencies_mock, patch.object(
            UsdUtils, "ModifyAssetPaths"
        ) as modify_assets_mock, patch.object(
            MaterialConverterUtils, "get_material_library_shader_urls"
        ) as get_shaders_mock, patch.object(
            OmniClientWrapper, "create_folder"
        ) as create_folder_mock, patch.object(
            OmniClientWrapper, "delete"
        ) as delete_folder_mock, patch.object(
            OmniClientWrapper, "copy"
        ) as copy_mock, patch.object(
            OmniUrl, "exists", new_callable=PropertyMock
        ) as exists_mock:
