# Changelog
The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

## [1.1.6]
### Changed
- Create one IngestCraft stage for eager and on-demand activation without deferred replacement work
- Use the native USD stage-opened lifecycle event instead of settings signals and fixed frame delays

## [1.1.5]
### Changed
- Updated extension metadata for Kit SDK 110 compatibility.

## [1.1.4]
### Changed
- Modernize python style and enable more ruff checks

## [1.1.3]
### Fixed
- Fixed workspace layout loading race condition on first load

## [1.1.2]
### Added
- Added QuickLayout dependency for layout management

### Changed
- Updated to support workspace-based architecture and flexible layouts

## [1.1.1]
## Fixed
- Fixed Test assets to large to work without LFS

## [1.1.0]
## Added
- Added missing automodule directives for documentation

## [1.0.6]
## Changed
- Update variables and resource locations for extension testing matrix (ETM) compliance

## [1.0.5]
## Changed
- Update to Kit 106.5

## [1.0.4]
### Fixed
- Fixed tests flakiness

## [1.0.3]
### Changed
- Changed repo link

## [1.0.2]
### Changed
- Update to Kit 106

## [1.0.1]
### Changed
- Set Apache 2 license headers

## [1.0.0] - 2024-02-28
### Added
- Created
