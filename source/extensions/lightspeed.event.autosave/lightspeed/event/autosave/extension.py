...
from .core import AutoSaveCore
from .preferences import AutoSavePreferencePage
...
class AutoSaveExtension(omni.ext.IExt):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.default_attr = {"_core": None, "_preferences_page": None}
        for attr, value in self.default_attr.items():
            setattr(self, attr, value)

    # noinspection PyUnusedLocal
    def on_startup(self, ext_id):
        carb.log_info("[lightspeed.event.autosave] Lightspeed Event Auto-Save startup")
        self._core = AutoSaveCore()
        _get_event_manager_instance().register_event(self._core)
        self._preferences_page = AutoSavePreferencePage()
        register_page(self._preferences_page)

    def on_shutdown(self):
        carb.log_info("[lightspeed.event.autosave] Lightspeed Event Auto-Save shutdown")
