# Changelog
The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

## [1.3.0]
### Changed
- Added optional edit lifecycle callbacks around StagePrimPicker dropdown sessions

## [1.2.1]
### Changed
- Modernize python style and enable more ruff checks

## [1.2.0]
### Changed
- Improved hover and selection styling to use TreeView's built-in styles for full-row highlighting
### Added
- Added initial selection highlighting when opening the dropdown

## [1.1.0]
### Added
- Added Escape key support to close dropdown

## [1.0.0]
### Added
- Initial release of Stage Prim Picker Widget in the form of a searchable dropdown, with infinite scroll support.
