...
class InterrupterBase(Protocol):
    @abc.abstractmethod
...
class EventOnShutdownBase(_ILSSEvent):
    def __init__(self):
        super().__init__()
        ...
        self._interrupters: list[InterrupterBase] = []
        self._currently_triggered_interrupters: list[InterrupterBase] = []

    @property
    def name(self) -> str:
        """Name of the event"""
        return "Generic OnShutdown Event"
    ...
    def _install(self):
        ...
        self._shutdown_event_sub = self._app.get_shutdown_event_stream().create_subscription_to_pop(
            self.__on_shutdown_event, name="lightspeed.event.shutdown_base hook", order=0
        )
    ...
    def register_interrupter(self, interrupter: InterrupterBase):
    ...
    def __shutdown_callback(self, interrupter):
        """Receive callback from an interrupter"""
