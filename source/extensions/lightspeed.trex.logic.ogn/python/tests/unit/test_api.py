...
class _TestLightspeedTrexLogicOgnApi(ogts.OmniGraphTestCase):
    ...
    async def test_api(self):
        _check_module_api_consistency(ogtp, self._UNPUBLISHED)  # noqa: PLW0212
        _check_module_api_consistency(ogtp.tests, is_test_module=True)  # noqa: PLW0212

    async def test_api_features(self):
        """Test that the known public API features continue to exist"""
        _check_public_api_contents(ogtp, [], self._UNPUBLISHED, only_expected_allowed=True)  # noqa: PLW0212
        _check_public_api_contents(ogtp.tests, [], [], only_expected_allowed=True)  # noqa: PLW0212
