...
class LockXformCore:
    ...
    def __init__(self):
        ...
        self._stage_event = omni.usd.get_context().get_stage_event_stream().create_subscription_to_pop(
            self._on_stage_event, name="[lightspeed.lock_xform] Stage Event")
        self._reason_for_lock = carb.settings.get_settings().get_as_string("/exts/lightspeed.lock_xform/reason_for_lock")
    ...
    def _on_stage_event(self, event):
        ...
        if event.type == int(omni.usd.StageEventType.OPENED):
            ...
            self._sessions[usd_context.get_stage_id()] = LockXformSession(usd_context.get_stage_id(), stage, self._display_dialog)
