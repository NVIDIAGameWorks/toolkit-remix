...
class TestRelativeReferences(AsyncTestCase):
    ...
    async def test_run_fix(self):
        ...
        def check_check_sub_validation(_result, _message, _data):
            ...
            if sub_check_count == 0:
                ...
                self.assertIn("Check", _message)
                self.assertIn("FAIL: /Xform/mesh", _message)
            else:
                self.assertTrue(_result, msg=_message)
                self.assertIn("Check", _message)
                self.assertNotIn("FAIL", _message)
        ...
        # Set reference path to absolute
        usd_context = omni.usd.get_context()
        stage = usd_context.get_stage()
        prim = stage.GetPrimAtPath("/Xform/mesh")
        prim.GetReferences().SetReferences(
            [
                Sdf.Reference(
                    assetPath=abs_ref_path,
                    primPath="/Cube",
                )
            ]
        )
        refs_and_layers = omni.usd.get_composed_references_from_prim(prim)
        self.assertEqual(Path(refs_and_layers[0][0].assetPath), Path(abs_ref_path))
