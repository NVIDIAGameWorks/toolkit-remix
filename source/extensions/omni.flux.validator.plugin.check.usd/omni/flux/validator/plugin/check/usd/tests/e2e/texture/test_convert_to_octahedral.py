...
from omni.flux.validator.plugin.check.usd.texture.convert_to_octahedral import ConvertToOctahedral
from omni.kit import ui_test
from omni.kit.test import AsyncTestCase
from omni.kit.test_suite.helpers import arrange_windows


class TestConvertToOctahedralUI(AsyncTestCase):
    """Exercise the regular ConvertToOctahedral plugin UI."""

    async def setUp(self):
        """Create the plugin's regular UI in a Kit window."""
        await arrange_windows()
        self._plugin = ConvertToOctahedral()
        self._data = ConvertToOctahedral.Data()
        self._window = ui.Window("TestConvertToOctahedralRegularUI", height=200, width=500)
    ...
    async def test_edit_max_workers_in_regular_ui_updates_schema(self):
        """Editing the regular MaxWorkersField updates ConvertToOctahedral data."""
