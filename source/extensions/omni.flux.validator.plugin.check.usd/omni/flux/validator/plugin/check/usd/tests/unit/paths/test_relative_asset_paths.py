...
class TestRelativeAssetPaths(AsyncTestCase):
    ...
    async def test_run_no_fix(self):
        ...
        def check_check_sub_validation(_result, _message, _data):
            ...
            self.assertIn("PASS: /Cube/OmniPBR", _message)
    ...
    async def test_run_fix(self):
        ...
        def check_check_sub_validation(_result, _message, _data):
            ...
            if sub_check_count == 0:
                ...
                self.assertIn("Check", _message)
                self.assertIn("FAIL: /Cube/OmniPBR/Shader", _message)
            else:
                self.assertTrue(_result, msg=_message)
                self.assertIn("Check", _message)
                self.assertNotIn("FAIL", _message)
                self.assertIn("PASS: /Cube/OmniPBR/Shader", _message)
        ...
        prim = stage.GetPrimAtPath("/Cube/OmniPBR/Shader")
        attr = prim.GetAttribute("inputs:diffuse_texture")

        # Set asset path to absolute
        attr.Set(attr.Get().resolvedPath)
        self.assertTrue(Path(str(attr.Get().path)).is_absolute())
    ...
    async def test_run_fix_in_reference(self):
        # Arrange
        await open_stage(get_test_data_path(__name__, "usd/subfolder/ref.usda"))
        core = _ManagerCore(
            {
                "name": "Test",
                "context_plugin": {"name": "CurrentStage", "data": {"context_name": ""}},
        ...
        def check_check_sub_validation(_result, _message, _data):
            ...
            if sub_check_count == 0:
                self.assertFalse(_result, msg=_message)
                self.assertIn("Check", _message)
                self.assertIn("FAIL: /Xform/mesh/OmniPBR/Shader", _message)
            else:
                self.assertTrue(_result, msg=_message)
                self.assertIn("Check", _message)
                self.assertNotIn("FAIL", _message)
                self.assertIn("PASS: /Xform/mesh/OmniPBR/Shader", _message)
        ...
        usd_context = omni.usd.get_context()
        stage = usd_context.get_stage()
        prim = stage.GetPrimAtPath("/Xform/mesh/OmniPBR/Shader")
        attr = prim.GetAttribute("inputs:diffuse_texture")
