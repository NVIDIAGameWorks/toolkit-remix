...
def on_click_open(dialog: FilePickerDialog, filename: str, dirname: str, callback: Callable):
    ...
    if not filename or str(Path(filename).suffix) not in [".usd", ".usda", ".usdc"]:
        carb.log_error("Please select an USD reference file")
        return
...
def open_file_picker(callback: Callable, callback_cancel: Callable, current_file: str = None, fallback=False):
    item_filter_options = ["USD Files (*.usd, *.usda, *.usdc)", "All Files (*)"]
    p_file = Path(current_file) if current_file else None
    navigate_to = current_file or None
    dialog = FilePickerDialog(
        ...
        current_directory=str(p_file.parent if p_file.is_file() else p_file) if p_file else None,
        current_filename=str(p_file.name) if p_file and p_file.is_file() else None,
