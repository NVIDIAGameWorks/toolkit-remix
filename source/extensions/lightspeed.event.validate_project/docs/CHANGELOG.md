# Changelog
The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).

## [1.1.7]
### Changed
- Coalesced stage and layer event bursts into one next-update project validation and silently skipped anonymous startup
  stages.

## [1.1.6]
### Changed
- Updated extension metadata for Kit SDK 110 compatibility.

## [1.1.5]
### Fixed
- Fixed crash when opening a project whose capture layer cannot be resolved on disk
### Added
- Added unit tests for unresolvable capture sublayer path

## [1.1.4]
### Changed
- REMIX-5202: Clarify notification message when an invalid capture reference is removed to indicate no files on disk are affected

## [1.1.3]
### Changed
- Updated LayerManagerCore calls

## [1.1.2]
### Changed
- Applied new lint rules

## [1.1.1]
## Fixed
- Fixed Test assets to large to work without LFS

## [1.1.0]
## Changed
- Update the documentation for Pydantic V2 compatbility

## [1.0.4]
### Fixed
- Fixed issue where layer validation was added to the undo stack

## [1.0.3] - 2024-05-07
### Changed
- Use updated `lightspeed.layer_manager.core` extension

### Fixed
- Cleanup lock state for invalid capture layers
- Use updated layer manager core ext

## [1.0.2]
### Changed
- Update to Kit 106

## [1.0.1]
### Changed
- Set Apache 2 license headers

## [1.0.0] - 2024-02-02
### Added
- Init commit.
