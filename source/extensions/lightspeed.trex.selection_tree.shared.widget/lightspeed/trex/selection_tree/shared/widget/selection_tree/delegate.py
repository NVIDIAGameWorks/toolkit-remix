...
class Delegate(ui.AbstractItemDelegate):
    ...
    def __init__(self):
        ...
        self._default_attr = {
            ...
            "_gradient_array_selected_hovered": None,
        ...
        self.__gradient_color1_selected_hovered = _hex_to_color(
            style.default["ImageWithProvider::SelectionGradient_selected_hovered"]["background_color"]
        )
        self.__gradient_color2_selected_hovered = _hex_to_color(
            style.default["ImageWithProvider::SelectionGradient_selected_hovered"]["background_gradient_color"]
        )
        ...
        self._gradient_array_selected_hovered = _create_gradient(
            self.__gradient_width,
            self.__gradient_height,
            self.__gradient_color1_selected_hovered,
            self.__gradient_color2_selected_hovered,
            (True, True, True, True),
        )

        self.__on_edit_path_reference = _Event()
    ...
    def _edit_path_reference(self, prim: "Usd.Prim", path: str):
        """Call the event object that has the list of functions"""
        self.__on_edit_path_reference(prim, path)

    def subscribe_edit_path_reference(self, function: Callable[["Usd.Prim", str], None]):
        """
        Return the object that will automatically unsubscribe when destroyed.
        """
        return _EventSubscription(self.__on_edit_path_reference, function)

    def _delete_reference(self, prim: "Usd.Prim", path: str):
        """Call the event object that has the list of functions"""
        self.__on_delete_reference(prim, path)
    ...
    def subscribe_toggle_visibility(self, function: Callable[["Usd.Prim", str], None]):
        """
        Return the object that will automatically unsubscribe when destroyed.
        """
    ...
    def subscribe_frame_prim(self, function: Callable[["Usd.Prim", str], None]):
        """
        Return the object that will automatically unsubscribe when destroyed.
        """
    ...
    def subscribe_toggle_nickname(self, function: Callable[["Usd.Prim", str], None]):
        """
        Return the object that will automatically unsubscribe when destroyed.
        """
                                ...
                                with self._path_scroll_frames[id(item)]:
                                    with ui.HStack():
                                        if isinstance(item, _ItemMesh):
                                            ...
                                            ui.Label(os.path.basename(item.path), name="PropertiesPaneSectionTreeItem")
                                        elif isinstance(item, _ItemAddNewReferenceFileMesh):
                    ...
                    if isinstance(item, _ItemMesh):
                        ...
                        with ui.VStack(
                            ...
                            ui.Image(
                                ...
                                height=ui.Pixel(16),
                                name="OpenFolder",
                                tooltip="Edit the path of the USD reference",
                                mouse_released_fn=lambda x, y, b, m: self._on_open_folder_mouse_released(b, item),
                            )
                            ui.Spacer(width=0)
                        ui.Spacer(height=0, width=ui.Pixel(8))
                        with ui.VStack(
                            ...
                            ui.Image(
                                ...
                                mouse_released_fn=lambda x, y, b, m: self._on_delete_ref_mouse_released(b, item),
                            )
                            ui.Spacer(width=0)
                        ui.Spacer(height=0, width=ui.Pixel(8))
                        with ui.VStack(
                            ...
                            ui.Image(
                                ...
                                height=ui.Pixel(16),
                                name="Eye",
                                tooltip="Hide/show the asset. Keep in mind that the exporter will ignore the value",
                                mouse_released_fn=lambda x, y, b, m: self._on_visibility_mouse_released(b, item),
                            )
                            ui.Spacer(width=0)
                    elif isinstance(item, _ItemInstanceMesh):
                        ui.Spacer(height=0, width=ui.Pixel(8))
    ...
    def _on_open_folder_mouse_released(self, button, item):
        if button != 0:
            return
        self._edit_path_reference(item.prim, item.path)

    def _on_delete_ref_mouse_released(self, button, item):
        if button != 0:
            return
        self._delete_reference(item.prim, item.path)
    ...
    def refresh_gradient_color(self, item):
        ...
        if is_hovered and is_selected and not self.__item_is_pressed:
            self._gradient_image_provider[id(item)].set_bytes_data(
                self._gradient_array_selected_hovered.ravel().tolist(), [self.__gradient_width, self.__gradient_height]
            )
        elif is_hovered and is_selected and self.__item_is_pressed:
            self._gradient_image_provider[id(item)].set_bytes_data(
                self._gradient_array_selected.ravel().tolist(), [self.__gradient_width, self.__gradient_height]
            )
        elif is_hovered and not self.__item_is_pressed:
            self._gradient_image_provider[id(item)].set_bytes_data(
                self._gradient_array_hovered.ravel().tolist(), [self.__gradient_width, self.__gradient_height]
            )
        elif not is_hovered and is_selected:
            self._gradient_image_provider[id(item)].set_bytes_data(
                self._gradient_array_selected.ravel().tolist(), [self.__gradient_width, self.__gradient_height]
            )
        else:
