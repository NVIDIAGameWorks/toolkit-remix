...
### Fixed
- Kept fixed-label-column property fields inside the property panel while resizing.
...
## [2.12.3]
### Changed
- Added default property edit interaction hooks to shared item models so delegates can use explicit model calls
...
## [2.10.4]
### Changed
- Added return type annotation `-> list[Item]` to `Model.get_item_children()` for clearer typing
...
## [2.10.2]
### Changed
- Updated tests to use `TreeItemBase.parent` setter for adding children (compatibility with omni.flux.utils.widget 1.24.0)
...
## [2.8.0]
## Added
- Added the ability to left-align the property names
