...
## [1.15.7]
### Changed
- Updated layer expansion controls to reflect existing child rows.

## [1.15.6]
### Added
- Added regression coverage for vertical layer popup menu placement.

### Fixed
- Removed the explicit horizontal direction from layer context menus so Kit SDK 110 uses the native vertical `ui.Menu` default.
...
## [1.15.3]
### Fixed
- Increased the active layer background tiling so the authoring hatch covers wider layer rows
...
## [1.14.0]
### Added
- Added automatic initial expansion of root layer and path to edit target
- Increased default tree frame height from 100 to 130
