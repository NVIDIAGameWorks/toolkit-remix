...
from lightspeed.trex.selection_tree.widget import SetupUI as _SetupUI
...
class TestSelectionTreeWidget(AsyncTestCase):
    ...
    async def test_nickname_double_click_edit(self):
        ...
        await ui_test.human_delay(human_delay_speed=3)
        fields = ui_test.find_all(f"{_window.title}//Frame/**/Label[*].identifier=='item_prim'")
        field = fields[-1]
        self.assertEqual(field.widget.text, "Cube_01")
        ...
        fields = ui_test.find_all(f"{_window.title}//Frame/**/Label[*].identifier=='item_prim'")
        field = fields[-1]
        self.assertEqual(field.widget.text, "New Nickname")
    ...
    async def test_nickname_toggle(self):
        ...
        fields = ui_test.find_all(f"{_window.title}//Frame/**/Label[*].identifier=='item_prim'")
        field = fields[-1]
        self.assertEqual(field.widget.text, "Test Nickname")
        ...
        # Click the toggle to hide nicknames — force_prim_name becomes True
        await nickname_toggle.click()
        await ui_test.human_delay(human_delay_speed=50)

        fields = ui_test.find_all(f"{_window.title}//Frame/**/Label[*].identifier=='item_prim'")
        field = fields[-1]
        self.assertNotEqual(field.widget.text, "Test Nickname")

        # Click the toggle again to show nicknames — force_prim_name becomes False
        await nickname_toggle.click()
        await ui_test.human_delay(human_delay_speed=50)
