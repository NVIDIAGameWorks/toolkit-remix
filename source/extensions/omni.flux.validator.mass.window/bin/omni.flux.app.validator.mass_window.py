...
import omni.usd
from omni.flux.validator.mass.window import ValidatorMassWindow as _ValidatorMassWindow
...
class StandaloneWindow:
    ...
    async def _dock(self) -> None:
        ...
        while True:
            await omni.kit.app.get_app().next_update_async()
            validation_window = ui.Workspace.get_window(_ValidatorMassWindow.WINDOW_NAME)
            if validation_window is not None:
                break
            frame += 1
            if frame == 100:
                raise TimeoutError(f"Can't set the workspace, missing {_ValidatorMassWindow.WINDOW_NAME} window")
