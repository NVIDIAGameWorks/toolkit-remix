...
import lightspeed.trex.control.stagecraft.unsaved_stage
import omni.kit.app
import omni.usd
...
class TrexTestPromptIfUnsavedStage(AsyncTestCase):
    ...
    def _mock_get_carb_setting(self, setting: str):
        from lightspeed.trex.control.stagecraft.setup import TREX_IGNORE_UNSAVED_STAGE_ON_EXIT
    ...
    async def test_shutdown_no_prompt_if_ignore_on_exit_setting_is_true(self):
        ...
        with (
            ...
            event_name = lightspeed.trex.control.stagecraft.unsaved_stage.EventUnsavedStageOnShutdown().name
